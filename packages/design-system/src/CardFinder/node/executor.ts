...
export default class CardFinderExecutor extends PromiseNode {
  ...
  protected async validateConfig(config: CardFinderConfig): Promise<ValidationResult> {
    return { success: true };
  }

  protected async executeNode(
    inputs: Record<string, any>,
    config: CardFinderConfig,
    context: NodeExecutionContext
  ): Promise<CardFinderOutput> {
    ...
    if (config.currentPhase !== undefined) {
      props.currentPhase = config.currentPhase;
    }
    ...
    this.logger.info(`✅ [CardFinder] ComponentSpec generated for node: ${context.nodeId}`);
