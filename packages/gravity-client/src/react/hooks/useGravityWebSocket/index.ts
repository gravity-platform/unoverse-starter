...
export function useGravityWebSocket(
  ...
  const [nodeExecutionEvent, setNodeExecutionEvent] = useState<any>(null); // For GravityCanvas debug
      ...
      ws.onmessage = (event) => {
        ...
        try {
          ...
          handleServerMessage(data, ws!, {
            ...
            removeComponent,
            setWorkflowState,
            setNodeExecutionEvent, // For GravityCanvas debug visualization
            sessionParams,
            onAudioState,
          });
        } catch (error) {
  ...
  return {
    ...
    isReady,
    events,
    nodeExecutionEvent, // For GravityCanvas debug visualization
    sendComponentReady,
    sendUserAction,
    loadTemplate,
    sendAgentMessage,
