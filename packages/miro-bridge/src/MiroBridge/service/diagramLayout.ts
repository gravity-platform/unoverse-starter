...
export function computeLayout(
  ...
  for (let l = 0; l < layers.length; l++) {
    for (let p = 0; p < layers[l].length; p++) {
      const id = layers[l][p];
      if (direction === "LR") {
        posMap.set(id, {
          x: l * (NODE_WIDTH + HORIZONTAL_GAP),
          y: p * (NODE_HEIGHT + VERTICAL_GAP),
        });
      } else {
        posMap.set(id, {
          x: p * (NODE_WIDTH + HORIZONTAL_GAP),
  ...
  const bounds =
    direction === "LR"
      ? {
          width: layerCount * (NODE_WIDTH + HORIZONTAL_GAP) - HORIZONTAL_GAP,
          height: maxNodesInLayer * (NODE_HEIGHT + VERTICAL_GAP) - VERTICAL_GAP,
        }
      : {
          width: maxNodesInLayer * (NODE_WIDTH + HORIZONTAL_GAP) - HORIZONTAL_GAP,
          height: layerCount * (NODE_HEIGHT + VERTICAL_GAP) - VERTICAL_GAP,
        };
