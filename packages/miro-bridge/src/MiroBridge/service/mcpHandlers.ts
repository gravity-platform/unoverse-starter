...
async function resolveParentId(
  ...
  if (frames.length > 0) return frames[frames.length - 1].id;

  // No frame exists — auto-create a default one
  const xOffset = key ? await nextFrameX(api, key, 1600) : 0;
  if (key) nextFrameSlot(api, key);
  const { id } = await apiCreateFrame(accessToken, boardId, {
    content: "Board",
    x: xOffset,
    y: 0,
    width: 1600,
  ...
  if (key) await applyCreate(api, key, { id, type: "frame", content: "Board", x: xOffset, y: 0, width: 1600, height: 900 });
  return id;
}

function resolveGridPosition(api: any, key: string | null, parentId: string): { x: number; y: number } {
  const slot = key ? nextItemSlot(api, key, parentId) : 0;
  const col = slot % 3;
  const row = Math.floor(slot / 3);
  return { x: 110 + col * 220, y: 110 + row * 220 };
}
...
export async function handleCreateSticky(
  ...
  const resolvedParentId = await resolveParentId(api, key, accessToken, boardId, params.parentId);
  const { x, y } = resolvedParentId ? resolveGridPosition(api, key, resolvedParentId) : { x: 0, y: 0 };
  const p = { content: params.text, x, y, width: params.width, style: { fillColor: params.color ?? "yellow" }, parentId: resolvedParentId };
  const { id } = await apiCreateSticky(accessToken, boardId, p);
  if (key) await applyCreate(api, key, { id, type: "sticky_note", content: params.text, x, y, parentId: resolvedParentId });
  return { ok: true, id };
}
...
export async function handleCreateText(
  ...
  const resolvedParentId = await resolveParentId(api, key, accessToken, boardId, params.parentId);
  let x = 100, y = 100;
  if (resolvedParentId && key) {
    const slot = nextTextSlot(api, key, resolvedParentId);
    x = 100;
    y = 100 + slot * 120;
  }
  const p = { content: params.text, x, y, style: { fontSize: params.fontSize }, parentId: resolvedParentId };
  const { id } = await apiCreateText(accessToken, boardId, p);
  if (key) await applyCreate(api, key, { id, type: "text", content: params.text, x, y, parentId: resolvedParentId });
  return { ok: true, id };
}
...
export async function handleCreateCard(
  ...
  const resolvedParentId = await resolveParentId(api, key, accessToken, boardId, params.parentId);
  const { x, y } = resolvedParentId ? resolveGridPosition(api, key, resolvedParentId) : { x: 0, y: 0 };
  const p = { content: params.title, x, y, style: { description: params.description, dueDate: params.dueDate }, parentId: resolvedParentId };
  const { id } = await apiCreateCard(accessToken, boardId, p);
  if (key) await applyCreate(api, key, { id, type: "card", content: params.title, x, y, parentId: resolvedParentId });
  return { ok: true, id };
}
...
export async function handleCreateAppCard(
  ...
  const resolvedParentId = await resolveParentId(api, key, accessToken, boardId, params.parentId);
  const { x, y } = resolvedParentId ? resolveGridPosition(api, key, resolvedParentId) : { x: 0, y: 0 };
  const p = { content: params.title, x, y, style: { description: params.description, status: params.status, fields: params.fields }, parentId: resolvedParentId };
  const { id } = await apiCreateAppCard(accessToken, boardId, p);
  if (key) await applyCreate(api, key, { id, type: "app_card", content: params.title, x, y, parentId: resolvedParentId });
  return { ok: true, id };
}
...
export async function handleCreateImage(
  ...
  const resolvedParentId = await resolveParentId(api, key, accessToken, boardId, params.parentId);
  const { x, y } = resolvedParentId ? resolveGridPosition(api, key, resolvedParentId) : { x: 0, y: 0 };
  const p = { content: params.url, x, y, width: params.width ?? 300, parentId: resolvedParentId };
  const { id } = await apiCreateImage(accessToken, boardId, p);
  if (key) await applyCreate(api, key, { id, type: "image", content: params.url, x, y, parentId: resolvedParentId });
  return { ok: true, id };
}
...
export async function handleCreateDiagram(
  ...
  if (params.parentId) {
    ...
    const frameWidth = Math.max(1600, layout.bounds.width + 200);
    const frameHeight = Math.max(900, layout.bounds.height + 200);
    const xOffset = key ? await nextFrameX(api, key, frameWidth) : 0;
    if (key) nextFrameSlot(api, key);
    const title = params.title ?? "Diagram";
    const { id: fid } = await apiCreateFrame(accessToken, boardId, {
      content: title, x: xOffset, y: 0, width: frameWidth, height: frameHeight,
    });
    if (key) await applyCreate(api, key, { id: fid, type: "frame", content: title, x: xOffset, y: 0, width: frameWidth, height: frameHeight });
    parentId = fid;
  }

  // Centre nodes within the frame — frame origin is frame centre, child coords are frame-relative
  const offsetX = -(layout.bounds.width / 2) + NODE_WIDTH / 2;
  const offsetY = -(layout.bounds.height / 2) + NODE_HEIGHT / 2;
  ...
  for (const node of layout.nodes) {
    ...
    const { id } = await apiCreateShape(accessToken, boardId, p);
    if (key) await applyCreate(api, key, { id, type: "shape", content: node.label, x: p.x, y: p.y, width: NODE_WIDTH, height: NODE_HEIGHT, parentId });
  ...
  return { ok: true, itemIds: allIds, parentId };
}
